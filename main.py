...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana amsa lokacin da mai amfani ya fara tattaunawa da bot."""
...
async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana dawo da farashin cryptocurrency."""
    if not context.args:
...
async def set_alert(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana saita faɗakarwar farashi ga wani coin."""
    if len(context.args) < 3:
        await update.message.reply_text(
            "Don Allah ka bayar da sunan coin, farashi, da kuma shugabanci (up/down).\n"
    ...
    try:
        ...
        await update.message.reply_text("Farashin da ka bayar ba lambar ba ce. Misali: /alert ethereum 3000 up")
        return
    ...
    try:
        ...
        price_alerts[chat_id][coin_id] = {
            ...
            'direction': direction,
            'original_coin_name': coin_name # Don nuna wa mai amfani da sunan da ya yi amfani da shi
        }
        await update.message.reply_text(
            f"An saita faɗakarwa don {coin_name.capitalize()}: "
            f"lokacin da farashin ya kai ${target_price:,.2f} ko kuma ya wuce shi ({direction})."
        )
    except Exception as e:
...
async def my_alerts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana nuna faɗakarwar farashi na mai amfani."""
    chat_id = update.effective_chat.id
    ...
    for coin_id, alert_info in price_alerts[chat_id].items():
        original_coin_name = alert_info.get('original_coin_name', coin_id) # Yi amfani da sunan asali idan akwai
        message += (
...
async def cancel_alert(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana soke faɗakarwar farashi ga wani coin."""
    if not context.args:
...
async def check_alerts(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana duba farashin kuma yana aika faɗakarwa idan an kai ga farashin."""
    for chat_id, alerts_for_user in list(price_alerts.items()): # Yi amfani da list don gujewa kuskure yayin gogewa
        for coin_id, alert_info in list(alerts_for_user.items()):
            try:
                params = {
                    "ids": coin_id,
                    "vs_currencies": "usd"
                ...
                if coin_id in data and 'usd' in data[coin_id]:
                    ...
                    if send_alert:
                        ...
                        # Cire faɗakarwar bayan an aiko ta
                        del price_alerts[chat_id][coin_id]
                        if not price_alerts[chat_id]: # Idan babu sauran alerts ga mai amfani, cire shigarsa
...
async def unknown(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Yana amsa saƙonnin da ba a fahimta ba."""
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("help", start)) # /help zai yi amfani da aikin /start
    ...
    # Task don duba faɗakarwa kowane minti 5 (seconds * minutes)
    job_queue = application.job_queue
    job_queue.run_repeating(check_alerts, interval=300, first=10) # Za a duba kowane minti 5

    # Amsa ga saƙonnin da ba a gane ba (dole ne ya zama na ƙarshe)
